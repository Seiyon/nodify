...
namespace TestWpfApplication.ViewModel
{
    ...
    public class StateMachineRunnerViewModel : ObservableObject
    {
        ...
        private readonly Runner.Debugging.DebugBlackboardDecorator _debugger = new Runner.Debugging.DebugBlackboardDecorator();
        private readonly Runner.Blackboard.Blackboard _original = new Runner.Blackboard.Blackboard();
        protected StateMachineViewModel StateMachineViewModel { get; }

        private Runner.MachineState _state;
        public Runner.MachineState State
        {
            get => _state;
        ...
        private void OnBlackboardKeyValueChanged(Runner.Blackboard.BlackboardKey key, object? newValue)
        {
            if (_stateMachine != null && _stateMachine.State != Runner.MachineState.Stopped)
            {
        ...
        private IEnumerable<State> CreateStates(IEnumerable<StateViewModel> states)
            => states.Select(s => new Runner.Debugging.DebugStateDecorator(new State(s.Id, CreateTransitions(s), CreateAction(s.Action))));
        ...
        private IEnumerable<Transition> CreateTransitions(StateViewModel state)
        {
            ...
            for (int i = 0; i < transitions.Count; i++)
            {
                ...
                result.Add(new Runner.Debugging.DebugTransitionDecorator(tr));
            }
